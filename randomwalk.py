...
def new_walk_gen(walkers=10, steps_per_frame=1):
    ...
    while True:
        for step in range(steps_per_frame):
            ...
            xs += np.where(np.logical_and(moves == EAST, np.logical_and(np.logical_and(np.logical_and(xs != -21, np.logical_and(ys > -21, ys <21))),np.logical_and(xs != 19, np.logical_or(np.logical_and(ys > -21, ys < -5)),np.logical_and(ys < 21, ys > 5)), 1, 0)
